"""
ParticleTwin (M6) — the real Twin. Replaces StubTwin.

Each cut: load the persisted TwinState, run one filter step against the observed
feature, derive an RUL from the posterior cloud, and persist the updated state.
Satisfies the same Twin.update() contract the handler already calls, so nothing
upstream changes — the flat stub line simply becomes real wear tracking.

RUL here projects each posterior wear particle to the threshold in CLOSED FORM
(the degradation model's cuts_to_threshold). M7 will replace this with a proper
Monte Carlo forward simulation that also accounts for FUTURE process noise.
"""
...
class ParticleTwin(Twin):
    def __init__(self, data_store, *, process_noise: float = 0.002,
                 resample_frac: float = 0.5, seed: int = 0) -> None:
        self.ds = data_store
    ...
    def update(self, run_id: str, cut_index: int, features: dict[str, float]) -> RULPrediction:
        ...
        rul_particles = deg.cuts_to_threshold(cloud.wear, threshold)
        rul_med = weighted_quantile(rul_particles, cloud.weights, 0.5)
        rul_lo = weighted_quantile(rul_particles, cloud.weights, 0.05)
        rul_hi = weighted_quantile(rul_particles, cloud.weights, 0.95)

        self.ds.save_twin_state(TwinState(run_id, cut_index, params=state.params,
                                          particles=cloud.to_bytes()))
        return RULPrediction(run_id, cut_index, rul_med, rul_lo, rul_hi, ci_level=0.9)
